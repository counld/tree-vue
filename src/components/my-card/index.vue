      ...
      <el-card body-style="padding: '0px'" shadow="hover">
        <div style="minwidth: 200px">
          ...
          <div class="bottom-style clearfix">
            ...
            <el-button type="text" class="button" @click="goDetails(item.id)">{{
              opration
            }}</el-button>
          </div>
        </div>
...
<script>
import { queryApiList, queryApiArticleHits, GetArticleNextList } from "@/utils/queryApi";
export default {
  name: "my-card",
  data() {
    return {
      ...
      if (top > height - 300 && this.isQueryOpen) {
        ...
        GetArticleNextList.call(this, this.getLastId).then((response) => {
          if (response.status == 200 && response.data.length) {
           ...
           if(response.data.length == 10) { //一次请求十条数据;
